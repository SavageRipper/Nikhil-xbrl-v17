...
xml_path=Path(sys.argv[1]) if len(sys.argv)>1 else Path('/mnt/data/charvak_1.xml')
...
# Golden validated XML has text facts without xml:lang; this must not be rejected by V17.
...
print(f'V17 FY 2024-25 XML regression: PASS ({len(contexts)} contexts, {len(facts)} fact occurrences, {len(typed_members)} typed members)')
